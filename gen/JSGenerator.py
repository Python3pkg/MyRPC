...
class JSGenerator(GeneratorBase):
    ...
    def _gen_exc_handler(self):
        ...
        for method in methods:
            ...
            sb.wl("\t\tdefault:")
            sb.wl("\t\t\tthrow new myrpc.common.DeserializerException(\"Exception \" + name + \" is unknown\");")
            sb.wl("\t}")
            sb.wl("};")
            sb.we()
    ...
    def _dtype_kind_enum_gen(self, dtype):
        ...
        sb.we()
        sb.wl("\t\tif (is_read)")
        sb.wl("\t\t\tthrow new myrpc.common.DeserializerException(msg);")
        sb.wl("\t\telse")
        sb.wl("\t\t\tthrow new myrpc.common.SerializerException(msg);")
        sb.wl("\t}")
        sb.wl("};")
        sb.we()
    ...
    def _dtype_kind_list_gen(self, dtype):
        ...
        sb.wl("\t\tthrow new myrpc.common.DeserializerException(\"List {} has unexpected elem data type \" + dtype);".format(dtype_name))
        sb.we()
    ...
    def _dtype_kind_struct_gen(self, dtype, classn = None):
        ...
        sb.wl("\t\t\tdefault:")
        sb.wl("\t\t\t\tthrow new myrpc.common.DeserializerException(\"Struct {} unknown fid \" + fid);".format(dtype_name))
        sb.wl("\t\t}")
        sb.we()
        sb.wl("\t\tif (err)")
        sb.wl("\t\t\tthrow new myrpc.common.DeserializerException(\"Struct {} fid \" + fid + \" has unexpected data type \" + dtype);".format(dtype_name))
        sb.we()
